...
function geocode() {
    ...
    geocode.prototype.initialize = function () {
        ...
        map = null;
        var address = discApp.getAddress();
        console.log(address);
        var mapOptions = {
            zoom: 15,
            center: address,
        ...
        infowindow = new google.maps.InfoWindow({
            content: 'Localización de la oferta'
        });
    ...
    var CoordenadasGPS = function (position) {
        ...
        console.log("Coordenadas: " + ini);
        discApp.setLocation(ini);
    ...
    var calcRoute = function (origin, destination) {
        console.log("Trazando ruta desde: " + origin);
        console.log("hasta: " + destination);
        var request = {
            origin: origin,
        ...
        directionsService.route(request, function (response, status) {
            if (status === google.maps.DirectionsStatus.OK) {
                ...
                discApp.showMessage('No se pudo realizar georeferenciación de la oOferta');
                jQuery('#panelError').show(); jQuery('#map').hide(); 
            }
            
            jQuery('#footerApp').hide(); discApp.hideCargando();
        });
    };
}